-- ==========================================
-- ULTIMATE PROJECT SETUP (Supabase)
-- ==========================================
-- This script contains:
-- 1. Database Schema (Tables, Enums, Extensions)
...
-- Create a public view for chat display
...
create table public.lessons (
  ...
  title text not null,
  description text,
  track_type text not null, -- Changed from Enum to Text for dynamic tracks
  level user_level,
...
-- User Tasks
create policy "Users and admins view user tasks" on user_tasks for select using (auth.uid() = user_id or exists (select 1 from profiles where id = auth.uid() and role = 'admin'));
create policy "Users insert own tasks" on user_tasks for insert with check (auth.uid() = user_id);
create policy "Users and admins update user tasks" on user_tasks for update using (auth.uid() = user_id or exists (select 1 from profiles where id = auth.uid() and role = 'admin'));

-- User Lessons
create policy "Users manage own lessons" on user_lessons for all using (auth.uid() = user_id);

-- Custom Lessons
create policy "Users view own custom lessons" on custom_lessons for select using (auth.uid() = user_id);
create policy "Admins manage custom lessons" on custom_lessons for all using (exists (select 1 from profiles where id = auth.uid() and role = 'admin'));
create policy "Users update own custom lessons" on custom_lessons for update using (auth.uid() = user_id);
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = user_email) THEN
    ...
    -- Directly insert into user_roles for the initial setup to be sure
